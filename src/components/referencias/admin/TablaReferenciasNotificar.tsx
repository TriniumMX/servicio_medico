import { Bell, Calendar, User, Phone, Clock, RefreshCcw, Stethoscope, AlertTriangle, CheckCircle } from "lucide-react";
...
export default function TablaReferenciasNotificar({ referencias, onNotificar, isDark }: Props) {
            ...
            <div key={`mob-${ref.id_referencia}`} className={`p-4 transition-colors ${isDark ? "hover:bg-white/5" : "hover:bg-gray-50/80"}`}
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 ${isDark ? "bg-[#0f83b2]/20 text-[#0db1ec]" : "bg-blue-50 text-blue-600"}`}>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {esSeg && (
                        <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-amber-500 text-white tracking-wide shrink-0">
                          ...
                          SEG{ref.numero_consulta ? ` #${ref.numero_consulta}` : ''}
                        </span>
                      )}
                    </div>
                ...
                <span className={`font-mono text-[10px] font-bold px-2 py-0.5 rounded-md shrink-0 ${isDark ? "bg-[#0f83b2]/10 text-[#0db1ec]" : "bg-blue-50 text-[#0f83b2]"}`}>
                  {ref.folio || "S/F"}
                </span>
              </div>
              ...
              <div className={`grid grid-cols-3 gap-2 mb-3 px-3 py-2.5 rounded-xl text-xs ${isDark ? "bg-[#0a1929]" : "bg-gray-50"}`}>
                <div>
                  ...
                  <p className={`font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>{formatDateShort(ref.fecha_autorizacion) ?? "—"}</p>
                ...
                <div>
                  <p className={`text-[9px] uppercase tracking-wide mb-1 font-bold ${isDark ? "text-gray-500" : "text-gray-400"}`}>Teléfono</p>
                  <p className={`font-medium truncate ${isDark ? "text-gray-300" : "text-gray-700"}`}>{ref.telefono || "—"}</p>
                </div>
              </div>
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${isDark
                  ? "bg-[#0f83b2] hover:bg-[#0d6f97] text-white"
        ...
        <div className={`flex items-center justify-between px-6 py-3 border-b text-xs ${isDark ? "border-[#0f83b2]/15 bg-[#0d2137]/60" : "border-gray-100 bg-gray-50/60"}`}>
          <div className="flex items-center gap-4">
            <span className={`flex items-center gap-1.5 font-semibold ${isDark ? "text-gray-400" : "text-gray-500"}`}>
              ...
              {referencias.length} referencias por notificar
            </span>
            <div className="flex items-center gap-3">
              {[
                { dot: "bg-red-500", label: "Urgente (≤3d)" },
                { dot: "bg-amber-500", label: "Próxima (≤30d)" },
                { dot: "bg-emerald-500", label: "Con tiempo" },
              ].map((l) => (
                <span key={l.label} className={`flex items-center gap-1.5 ${isDark ? "text-gray-500" : "text-gray-400"}`}>
        ...
        <table className="w-full">
          <thead>
            <tr className={`text-left text-[10px] font-bold uppercase tracking-widest border-b ${isDark ? "bg-[#0d2137] border-[#0f83b2]/15 text-gray-500" : "bg-gray-50 border-gray-100 text-gray-400"}`}>
              ...
              <th className="px-5 py-3">Médico asignado</th>
            ...
            {referenciasSorted.map((ref) => {
              ...
              return (
                <tr key={ref.id_referencia}
                  className={`group transition-colors ${isDark ? "hover:bg-[#0d2137]/70" : "hover:bg-blue-50/20"}`}
                  style={getTriageRowStyle(ref.nivel_triage, isDark)}>

                  {/* Paciente */}
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 transition-transform group-hover:scale-105 ${isDark ? "bg-[#0f83b2]/20 text-[#0db1ec]" : "bg-blue-50 text-blue-600"}`}>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`font-mono text-[10px] font-bold ${isDark ? "text-[#0db1ec]/70" : "text-[#0f83b2]/70"}`}>{ref.folio || "S/F"}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`text-sm font-medium ${isDark ? "text-gray-200" : "text-gray-700"}`}>{ref.nombre_especialidad}</span>
                    </div>
                    {triage && (
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm ${isDark ? "text-gray-300" : "text-gray-700"}`}>{ref.nombre_medico_asignado || "—"}</span>
                    </div>
                    {ref.fecha_cita && (
                      <p className={`text-[10px] mt-0.5 ml-5 ${isDark ? "text-gray-600" : "text-gray-400"}`}>
                        Cita: {formatDate(ref.fecha_cita)}
                      </p>
                    )}
                  </td>
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={`text-sm font-medium ${isDark ? "text-gray-200" : "text-gray-800"}`}>
                            {formatDate(ref.fecha_autorizacion)}
                          </span>
                        </div>
                        {diasAuth !== null && (
                          <p className={`text-[10px] mt-0.5 ml-5 ${isDark ? "text-gray-500" : "text-gray-400"}`}>
                            hace {diasAuth === 0 ? "hoy" : `${diasAuth} día${diasAuth > 1 ? "s" : ""}`}
                          </p>
                        )}
                      </div>
                    ) : (
                  ...
                  <td className="px-5 py-4">
                    {ref.telefono ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-sm font-mono ${isDark ? "text-gray-300" : "text-gray-700"}`}>{ref.telefono}</span>
                      </div>
                    ) : (
                      <span className={`text-xs ${isDark ? "text-gray-600" : "text-gray-400"}`}>—</span>
                    )}
                  ...
                  <td className="px-5 py-4 text-center">
                    <button
                      onClick={() => onNotificar(ref)}
                      className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm hover:shadow-md hover:scale-105 active:scale-95 ${isDark
                        ? "bg-[#0f83b2] hover:bg-[#0d6f97] text-white shadow-blue-900/20"
                        : "bg-[#0f83b2] hover:bg-[#0a7aa0] text-white shadow-blue-200"}`}
                    >
                      <Bell className="w-3.5 h-3.5" />
                      Notificar
                    </button>
